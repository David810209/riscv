...
static inline int check_case(unsigned idx, unsigned in, unsigned expect){
    unsigned out;
    __asm__ volatile ("orc.b %0, %1" : "=r"(out) : "r"(in)); 
    if (out == expect){
...
int main(void){
    ...
    fail |= check_case(1, 0x00000000u, 0x00000000u);
    fail |= check_case(2, 0x00000001u, 0x000000ffu);
    fail |= check_case(3, 0x00000100u, 0x0000ff00u);
    fail |= check_case(4, 0x01020304u, 0xffffffffu);
    fail |= check_case(5, 0x80808080u, 0xffffffffu);
    fail |= check_case(6, 0x80000000u, 0xff000000u);
    fail |= check_case(7, 0x00ff0001u, 0x00ff00ffu);
