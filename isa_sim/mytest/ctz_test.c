...
static inline int check_case(unsigned idx, unsigned in, unsigned expect){
    unsigned out;
    __asm__ volatile ("ctz %0, %1" : "=r"(out) : "r"(in)); 
    if (out == expect){
...
int main(void){
    ...
    fail |= check_case(1, 0x8524FA00u, 0x00000009u);
    fail |= check_case(2, 0x00000000u, 0x00000020u);
    fail |= check_case(3, 0x00000001u, 0x00000000u);
