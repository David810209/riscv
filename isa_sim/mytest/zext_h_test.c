...
static inline int check_case(unsigned idx, unsigned in, unsigned expect){
    unsigned out;
    __asm__ volatile ("zext.h %0, %1" : "=r"(out) : "r"(in)); 
    if (out == expect){
...
int main(void){
    sim_puts("ZEXT_H test\n");
    int fail = 0;
    fail |= check_case(1, 0x00007FFFu, 0x00007FFFu);
    fail |= check_case(2, 0x00008000u, 0x00008000u);
    fail |= check_case(3, 0x0000F000u, 0x0000F000u);
    fail |= check_case(4, 0xFFFFAAAAu, 0x0000AAAAu);
    fail |= check_case(5, 0xFFFFFFFFu, 0x0000FFFFu);
