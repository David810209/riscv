...
static inline int check_case(unsigned idx, unsigned in,unsigned in2, unsigned expect){
    unsigned out;
    __asm__ volatile ("bext %0, %1, %2" : "=r"(out) : "r"(in), "r"(in2)); 
    if (out == expect){
...
int main(void){
    sim_puts("BEXT test\n");
    int fail = 0;
    fail |= check_case(1, 0x00000000u, 0x00000000u, 0x00000000u);
    fail |= check_case(2, 0x00000002u, 0x00000000u, 0x00000000u);
    fail |= check_case(3, 0x00000001u, 0x00000000u, 0x00000001u);
    fail |= check_case(4, 0x80000000u, 0x0000001Fu, 0x00000001u);
    fail |= check_case(5, 0x7FFFFFFFu, 0x0000001Fu, 0x00000000u);
