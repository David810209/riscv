...
static inline int check_case(unsigned idx, unsigned in,unsigned in2, unsigned expect){
    unsigned out;
    __asm__ volatile ("binv %0, %1, %2" : "=r"(out) : "r"(in), "r"(in2)); 
    if (out == expect){
...
int main(void){
    ...
    fail |= check_case(2, 0x00000101u, 0x00000004u, 0x00000111u);
    fail |= check_case(3, 0x00000009u, 0x00000003u, 0x00000001u);
    fail |= check_case(4, 0x00000000u, 0x00000000u, 0x00000001u);
