...
static inline int check_case(unsigned idx, unsigned in,unsigned in2, unsigned expect){
    unsigned out;
    __asm__ volatile ("ror %0, %1, %2" : "=r"(out) : "r"(in), "r"(in2)); 
    if (out == expect){
...
int main(void){
    sim_puts("ROR test\n");
    int fail = 0;
    fail |= check_case(1, 0x00000001u, 0x00000000u, 0x00000001u);
    fail |= check_case(2, 0x00000001u, 0x00000001u, 0x80000000u);
    fail |= check_case(3, 0x00000001u, 0x00000007u, 0x02000000u);
    fail |= check_case(4, 0x00000001u, 0x0000000Eu, 0x00040000u);
    fail |= check_case(5, 0x00000001u, 0x0000001Fu, 0x00000002u);
    fail |= check_case(6, 0xFFFFFFFFu, 0x00000007u, 0xFFFFFFFFu);
    fail |= check_case(7, 0x21212121u, 0x00000001u, 0x90909090u);
    fail |= check_case(8, 0x21212121u, 0x00000007u, 0x42424242u);
    fail |= check_case(9, 0x12345678u, 0x00000004u, 0x81234567u);
