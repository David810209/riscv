...
static inline int check_case(unsigned idx, unsigned in,unsigned in2, unsigned expect){
    unsigned out;
    __asm__ volatile ("bclr %0, %1, %2" : "=r"(out) : "r"(in), "r"(in2)); 
    if (out == expect){
...
int main(void){
    sim_puts("BCLR test\n");
    int fail = 0;
    fail |= check_case(1, 0x00000020u, 0x00000005u, 0x00000000u);
    fail |= check_case(2, 0x00000101u, 0x00000000u, 0x00000100u);
    fail |= check_case(3, 0x00000009u, 0x00000003u, 0x00000001u);
    fail |= check_case(4, 0x00000011u, 0x00000004u, 0x00000001u);
    fail |= check_case(5, 0x00000101u, 0x00000008u, 0x00000001u);
